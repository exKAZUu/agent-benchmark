...
describe("cli", () => {
  ...
  test("calc prints error when operator is invalid", async () => {
    const result = await runCli(["calc", "2", "invalid", "3"]);
    expect(result.exitCode).toBe(1);
    expect(result.stderr).toContain("error: operator must be one of");
  });

  test("calc prints error when operands are not numbers", async () => {
    const result = await runCli(["calc", "abc", "+", "3"]);
    expect(result.exitCode).toBe(1);
    expect(result.stderr).toContain("error: left and right must be numbers");
  });
